...
var Editable = Class.create({
	initialize: function(element, options) {
		...
		this.value = this.element.innerHTML.strip();
	...
	setupForm: function() {
		...
		if(this.editField.type == 'textarea'){
			this.editField.element.setStyle({height: this.element.getStyle('height'), width: this.element.getStyle('width')});
		}
		...
		if(this.editField.type == 'select') {
			...
			try {
				this.editField.element.selectedIndex = $A(this.editField.element.options).find(function(option) {
					return option.text == this.element.innerHTML.strip();
				}.bind(this)).index;
			} catch(e) {
			...
			this.editField.element.value = this.element.innerHTML.strip();
		}
	},
